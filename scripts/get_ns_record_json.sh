...
# NS 레코드 캡처
NS_RECORDS=($(aws route53 list-resource-record-sets --hosted-zone-idd $ZONE_ID --query "ResourceRecordSets[?Type == 'NS' && Name == '$RECORD_NAME.$ZONE_NAME'].ResourceRecords[].Value" --output text))
